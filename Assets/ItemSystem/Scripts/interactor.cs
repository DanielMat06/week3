...
public class Interactor : MonoBehaviour
{
    ...
    public void Interact()
    {
        if (Input.GetKeyDown(KeyCode.F))
        {
            ...
            maxDistanceFromCamera, layerMask))
            {
                if (raycastHit.transform != null)
                {
                    distanceFromActor = Vector3.Distance(transform.position,
                    raycastHit.transform.position);
                    if (distanceFromActor <= maxInteractableDistance)
                    {
                        Debug.Log("In range: " + raycastHit.transform.name + " (" +
                        distanceFromActor.ToString("0.00") + " units)");
                        ItemNew1 item = raycastHit.transform.GetComponent<ItemNew1>();
                        if (item != null)
                        {
                            item.Interact();
                        }
