...
public class EnemyAttack : MonoBehaviour
{
    ...
    void Attack()
    {
        distanceToTarget = Vector3.Distance(playerTransform.position,
        ...
        if (distanceToTarget <= maxDistanceToTarget)
        {
            ...
            if (attackReady)
            {
                ...
                maxDistanceToTarget))
                {
                    Debug.DrawRay(gunTransform.transform.position,
                    gunTransform.transform.forward * 3,
                    Color.magenta, 0.2f);
                    Debug.Log("Enemy Shoots");
                    if (raycastHit.transform != null)
                    {
                        raycastHit.collider.SendMessageUpwards("Hit",
                        rawDamage,
